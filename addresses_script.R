...
get_all_profile_info <- function(st_pc_url) {
  ...
  acog_names_title <- st_pc_url |>
    ...
    separate(names_title, c("Names", "Title"),sep = ", ", 
             extra = "merge", fill = "right")
...
test_obgyn_ind_2_addresses_url <- "https://www.acog.org/womens-health/find-an-ob-gyn/results?firstname=&lastname=&address=83238&searchradius=10"

test_obgyn_ind <- "https://www.acog.org/womens-health/find-an-ob-gyn/physician?id=4f42a862-45c6-483e-86a7-712895663dca"

test_obgyn_ind_2_addresses <- test_obgyn_ind_2_addresses_url |> 
  read_html() |> 
  html_element(".nobull") |> 
  html_elements("li") |> 
...
test_obgyn_ind_2_addresses |> 
  mutate(obgyn_insurance = future_map(test_obgyn_ind_2_addresses, html_text2)) |>
...
get_insurance_info <- function(obgyn_url){
  ...
  contact_info <- obgyn_url |>
    ...
    html_element("address") |>
    html_text2() |>
    enframe(name = "ins_id", value = "ind_address") |>
    separate(
      ind_address,
      c("Contact Info", "Insurance 1", "Insurance 2"),
      sep = "\\sAccepts",
      extra = "merge",
      fill = "right"
    ) |>
